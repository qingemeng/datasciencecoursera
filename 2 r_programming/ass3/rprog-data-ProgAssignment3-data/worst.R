...
worst <- function(state, outcome) 
{
    ...
    maxRate <- max(as.double(as.character(rates)), na.rm=TRUE)
    #return(minRate)
    #print(dataState[,index])
    #return(minRate)
    
    record <- dataState[as.double(as.character(dataState[,index])) == maxRate,]#$Hospital.Name[1]
    record <- record[!is.na(record$Hospital.Name),]
    #return(record)
    tail(sort(as.character(record$Hospital.Name)), n=1)
